...
generic-misconfigured = Сервер неправильно налаштований і не приймає гравців. Будь ласка, зв'яжіться з власником сервера та спробуйте пізніше
ipintel-server-ratelimited = Ви не забанені. Ця гра використовує зовнішню перевірку, яка досягла максимального ліміту верифікацій нових підключень. Зачекайте хвилину-дві та підключіться знову; апеляція не потрібна. Якщо це не допоможе, спробуйте іншого дня або створіть тікет.
...
ipintel-suspicious = Ви підключаєтесь через дата-центр або VPN. Це не бан вашого акаунта, достатньо вимкнути VPN. Якщо у вас все ще є технічні проблеми або вам потрібен VPN для гри, ви можете подати запит на виняток за посиланням https://discord.gg/FtsCESsrzD
